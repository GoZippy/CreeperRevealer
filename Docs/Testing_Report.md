...
- test_batch_process.py: Validated orchestrator calls scripts in correct order and handles subprocess outputs.

Notes:
- Tests use lightweight mock modules for heavy dependencies (`cv2`, `torch`, `dlib`, `open3d`, `midas.model_loader`) and `subprocess.run` to ensure CI compatibility.
